...
Served from [`docs/`](docs/) by GitHub Pages, and it **answers questions live in
the browser**. Name a provision — "IPC 302", "BNSS Section 173", "Section 65B of
the Indian Evidence Act" — or describe an offence, and it resolves the
correspondence, reads the First Schedule and quotes the gazette, with a link to
the source for every claim.

The Python bot cannot run on a static host: it needs PyTorch, FAISS and a 300 MB
checkpoint. Its *trustworthy* half needs none of them — citation parsing, the
concordance lookup, the First Schedule and the statutory text are lookups over
verified data — so that half is ported to JavaScript and answers from
`docs/engine.json` (2.5 MB, about 700 KB gzipped, built by
[`scripts/build_engine.py`](scripts/build_engine.py)).

Two things the browser version deliberately does not do. It does not rank
passages by meaning, since the embeddings cannot come along, so a question naming
no section falls back to searching headings. And **nothing on the page is
generated by the fine-tuned model**, whose prose drifts section numbers and
corrupts titles; every sentence is composed from the data.

`node tests/test_site_engine.js` exercises the answering logic headlessly — 20
checks covering citation parsing, each answer branch, the scope refusals and
